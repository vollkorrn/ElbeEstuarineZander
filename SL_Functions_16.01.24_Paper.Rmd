...
col.Palette<-list(
  SL  = c( "SLSU21MG" = "steelblue","SLSU21BB" = "#9E9E9E","SLSU21SS" = "#FFA726", "SLSU21ML" ="#FF7043",  
           "WF" = "darkblue"),
  Kind = c("WF" = "darkblue", "SL" = "steelblue"),
  col.Palette.Locs    =c("Medem Grund" = "steelblue", "Brunsbuettel" = "#9E9E9E", 
                         "Schwarztonnen Sand" = "#FFA726", "Muehlenberger Loch" = "#F8766D"), 
  col.Palette.LOC    =c("MG-713" = "steelblue", "BB-692" = "#9E9E9E", "SS-665" = "#FFA726", "ML-633" = "#F8766D"), 
  col.Palette.Reps    =c("MG-713" = "steelblue", "BB-692" = "#9E9E9E", "SS-665" = "#FFA726", "ML-633" = "#F8766D", 
                         "WF" = "darkblue"),
  col.Palette.species = c("SL" = "seashell4"), 
  col.Palette.Cruises    =c("Summer_21" = "goldenrod1"), # "Summer_22" = "brown1"),
  col.Palette.Seasons    =c("Summer"    = "goldenrod1"),
  col.Palette.Season     =c("Elbe_Summer_21" = "goldenrod1"))
...
BacteriaHeatPlotRK6 <- function (res = res, vst = vst, Species = Species, min_count = 10, genes_of_interest = genes_of_interest, Samples = Samples, SAMDF = SAMDF,  TITLE = TITLE, filename= FILENAME) {
  ...
  CruiseOrder <-c("Summer_21", "Autumn_21", "Winter_22", "Spring_22", "Summer_22")
  SeasonOrder <-c("Summer", "Autmn", "Winter", "Spring")
  
  LocOrder=c("Medem Grund", "Oste", "Brunsbuettel", "Schwarztonnen Sand", "Muehlenberger Loch")
  LocOrderGC <- c("Oste", "Brunsbuettel", "Schwarztonnen Sand", "Muehlenberger Loch")
  
   if (gg == "GC" ) {
    col.Palette$col.Palette.Locs <- col.Palette$col.Palette.LocsGC } else 
    {col.Palette$col.Palette.Locs <- col.Palette$col.Palette.Locs}
    
    if (gg == "GC" ) {LocOrder <- LocOrderGC } else {LocOrder <- LocOrder}
  
  metadata$fLoc <- factor(metadata$Loc, levels=LocOrder)
  metadata$fCruise <- factor(metadata$Cruise, levels=CruiseOrder)
...
BacteriaHeatPlotRK7_withCore <- function (OmicsData = OmicsData, Species = Species, genes_of_interest = genes_of_interest, Samples = Samples, SAMDF = SAMDF,  TITLE = TITLE, filename= FILENAME, OutlineColor=OutlineColor) {
  paste("Code Raphael Koll raphael.koll@uni-hamburg.de code adapted from https://github.com/kevinblighe/E-MTAB-6141")
  require(RColorBrewer)
  require(ComplexHeatmap)
  require(Cairo)
  ...
  metadata$SampleID <- factor(metadata$SampleID, levels = SL_SSU_Samples)
  
  #create top-annotation
  CruiseOrder <-c("Summer_21", "Autumn_21", "Winter_22", "Spring_22", "Summer_22")
  SeasonOrder <-c("Summer", "Autmn", "Winter", "Spring")
  
  LOCOrder=c("MG-713", "BB-692", "SS-665", "ML-633")
  ...
  colAnn <-HeatmapAnnotation(
    ...
    which = 'col', # 'col' (samples) or 'row' (gene) annotation?
    na_col = OutlineColor, # default colour for any NA values in the annotation data-frame, 'ann'
    col = list(Location = col.Palette$col.Palette.LOC,
               Season = col.Palette$col.Palette.Cruises), 
    annotation_height = 0.4,
    annotation_width = unit(0.5, 'cm'),
    gap = unit(1, 'mm'),
   ...
   #############################
    ...
    for (x in 1:dim(heat)[1]) {
      if (rownames(heat)[x] %in% pslist$Core[c("ASV", "Phylum", "Order")]$ASV) {
      CoreTaxa[[x]] <- 1
...
GeneGSEAPlotKEGGandWikiRKwhiteSLUC_Kegg_Online <- function (tpm = tpm, vst = vst, Species = Species, tpm_value = 1, Samples = Samples, SAMDF = SAMDF,  TITLE = TITLE, filename= FILENAME, Comparisons = Comparisons) {
  tryCatch({
  ...
  sig_GO <- lapply(sig, function(x) {x[!duplicated(x$GeneSymbols),] }) #Remove duplicated
  ...
  tryCatch({
        ...
        gseKEGG <- list("gseKEGG.Liver" = gseKEGGres2[names(gseKEGGres2)[grepl("Liver", names(gseKEGGres2))]], 
                 "gseKEGG.Gill" = gseKEGGres2[names(gseKEGGres2)[grepl("Gill", names(gseKEGGres2))]])
    ...
    legend.box.background = element_rect(fill='transparent')) + 
        ...
        draw_label_themeRKwhite(paste(TITLE, Species, gg, Type, names(gseKEGGres2)[[x]]), 
        ...
        ggsave(AA, filename = paste(paste(save_name, Type, gg, paste("GSEA.KEGG",FILENAME, names(gseKEGGres2)[[x]], sep="_"), sep="_") ,".png", sep=""), path = pathPlots , 
        ...
        gseWIKI <- list("gseWIKI.Liver" = gseWPres2[names(gseWPres2)[grepl("Liver", names(gseWPres2))]], 
                 "gseWIKI.Gill" = gseWPres2[names(gseWPres2)[grepl("Gill", names(gseWPres2))]])
    ...
    legend.box.background = element_rect(fill='transparent')) + ylab("") + xlab("Normalized Enrichtment Score (NES)") 
         ...
         prow <- cowplot::plot_grid(g, labels = c(""), ncol = 1)
        title <- cowplot::ggdraw() +
        draw_label_themeRKwhite(paste(TITLE, Species, gg, Type, names(gseWPres2)[[xx]]), 
                       element = "plot.title",x = 0.05, hjust     = 0, vjust = 1)
        subtitle <- cowplot::ggdraw() + 
        ...
        if (OperatingSystem == "Mac" ) {quartz()}
        AA<- print(cowplot::plot_grid(title, subtitle, prow, ncol = 1, rel_heights = c(0.04, 0.05, 0.98)))
        ggsave(AA, filename =    paste(paste(save_name, Type, gg, paste("GSEA.WIKI",FILENAME, names(gseWPres2)[[xx]], sep="_"), sep="_") ,".png", sep=""), path = pathPlots , 
               device='png', dpi=300, width = 10,height = 10)},
        error=function(e){cat("ERROR :",conditionMessage(e), "\n")})}
    ...
    legend.box.background = element_rect(fill='transparent')) + ylab("") + xlab("Normalized Enrichtment Score (NES)") 
        ...
        prow <- cowplot::plot_grid(g, labels = c(""), ncol = 1)
        title <- cowplot::ggdraw() +
        draw_label_themeRKwhite(paste(TITLE, Species, gg, Type, names(gseRES)[[xxx]]), 
                       element = "plot.title",x = 0.05, hjust     = 0, vjust = 1)
        subtitle <- cowplot::ggdraw() + 
        ...
        if (OperatingSystem == "Mac" ) {quartz()}
        AA<- print(cowplot::plot_grid(title, subtitle, prow, ncol = 1, rel_heights = c(0.04, 0.05, 0.98)))
        ggsave(AA, filename =          paste(paste(save_name, Type, gg, paste("KEGGandWIKI.GSEA",FILENAME, names(gseRES)[[xxx]], sep="_"), sep="_") ,".png", sep=""), path = pathPlots , 
               device='png', dpi=300, width = 10,height = 10)},
        error=function(e){cat("ERROR :",conditionMessage(e), "\n")})}
...
.GlobalEnv[[name]] <- c(.GlobalEnv[[name]], gseKEGG)
.GlobalEnv[[name]] <- c(.GlobalEnv[[name]], gseWIKI)
.GlobalEnv[[name]] <- c(.GlobalEnv[[name]], gseKEGGandWIKI)
}
```
...
GeneHeatPlotRK5_SL <- function (res = res, vst = vst, Species = Species, genes_of_interest = genes_of_interest, tpm = tpm, tpm_value = tpm_value, Samples = Samples, SAMDF = SAMDF,  TITLE = TITLE, filename= FILENAME, cluster_columns = CLUSTER_COLUMNS) {
  ...
  SeasonOrder <-c("Elbe_Summer_21", "Elbe_Autumn_21" , "Elbe_Spring_22","Elbe_Winter_22","Elbe_Summer_22","Oste_Summer_22")

  LocOrder <- c("Medem Grund", "Oste", "Brunsbuettel", "Schwarztonnen Sand",  "Muehlenberger Loch")
  
  metadata$fLoc <- factor(metadata$Loc, levels=LocOrder)
  metadata$fSeason <- factor(metadata$Season, levels=SeasonOrder)
...
GeneHeatPlotRKnoClust_SL_tpm <- function (Species = Species, Input = Input, Samples = Samples, SAMDF = SAMDF,  TITLE = TITLE, filename= FILENAME2) {
  ...
  metadata   <- metadata[order(match(metadata$SampleID, colnames(heat))), , drop = FALSE]
  #create top-annotation
  CruiseOrder <-c("Summer_21")
  
  SeasonOrder <-c("Elbe_Summer_21") 
...
```{r}
BacteriaHeatPlotRKnoClust_SL <- function (res = res, clr = clr, Species = Species, min_count = 10, genes_of_interest = genes_of_interest, Samples = Samples, SAMDF = SAMDF,  TITLE = TITLE, filename= FILENAME, OutlineColor = OutlineColor) {
  paste("Code Raphael Koll raphael.koll@uni-hamburg.de code adapted by https://github.com/kevinblighe/E-MTAB-6141")
  require(RColorBrewer)
  require(ComplexHeatmap)
  require(Cairo)
...
WGCNA_IntegratedHeatmap_RK <- function (
  ...
  #create heatmap
    ...
    # row (gene) parameters
      ...
      # specify top and bottom annotations
      top_annotation = colAnn, 
      show_heatmap_legend = TRUE) -> RNA_hmap
      #bottom_annotation = boxplotCol) 
  ...
  ComplexHeatmap::Heatmap(
    ...
    if(SSU_corr_RNA$p_value[ii, j] < 0.001) {
        grid.text("***", x, y)
    } else if(SSU_corr_RNA$p_value[ii, j] < 0.01 & (SSU_corr_RNA$p_value[ii, j] > 0.001)) {
        grid.text("**", x, y)
    } else if(SSU_corr_RNA$p_value[ii, j] < 0.05 & (SSU_corr_RNA$p_value[ii, j] > 0.01)) {
        grid.text("*", x, y)
    }}, 
    col = colorRamp2(myBreaks, myCol),
      cluster_rows = FALSE,
    ...
    heatmap_legend_param = list(
      ...
      clustering_method_columns = 'ward.D2',
      clustering_distance_rows = function(x) as.dist(1 - cor(t(x))),
      clustering_method_rows = 'ward.D2') -> SSU_corr_RNA_plot
  
  #####################
  # Cor-Mat RNA-Traits#
  #####################
  #print("Cor-Mat RNA-Traits")
  ...
  ComplexHeatmap::Heatmap(
    ...
    if(Traits_corr_RNA$p_value[iiii, jjj] < 0.001) {
        grid.text("***", x, y)
    } else if(Traits_corr_RNA$p_value[iiii, jjj] < 0.01 & (Traits_corr_RNA$p_value[iiii, jjj] > 0.001)) {
        grid.text("**", x, y)
    } else if(Traits_corr_RNA$p_value[iiii, jjj] < 0.05 & (Traits_corr_RNA$p_value[iiii, jjj] > 0.01)) {
        grid.text("*", x, y)
    }
  }, 
    col = colorRamp2(myBreaks, myCol),
    ...
    heatmap_legend_param = list(
      ...
      clustering_method_columns = 'ward.D2',
      clustering_distance_rows = function(x) as.dist(1 - cor(t(x))),
      clustering_method_rows = 'ward.D2') -> Traits_corr_RNA_plot
  ...
  prow <- cowplot::plot_grid(grid.grabExpr(ComplexHeatmap::draw(ht_list, auto_adjust = FALSE, background = "transparent",
